...
if ($controls->is_action('test') || $controls->is_action('save') || $controls->is_action('send') || $controls->is_action('editor')) {
    ...
    // plugin addition
    if (isset($_POST['customers_listing'])) {
        $customers = $_POST['customers_listing'];
        if (is_array($customers) && count($customers) > 0) {
            $allInts = array_map('intval', $customers);
            $ids = implode(',', $allInts);
    ...
    <form method="post" action="" id="newsletter-form">
        ...
        <div id="tabs">
            ...
            <div id="tabs-c">
                <table class="form-table">
                    <tr valign="top">
                        <th><?php _e('Customers', 'newsletter'); ?></th>
                        <td>
                            <table id="from-to-to" class="no-pad">
                                <tr>
                                    <td><label for="customer_from">From: </label></td>
                                    <td><label for="customer_to">To: </label></td>
                                </tr>
                                <tr id="customer_form">
                                    <td><input class="datepicker" type="text" name="customer_from" id="customer_from"></td>
                                    <td><input class="datepicker" type="text" name="customer_to" id="customer_to"></td>
                                    <td>
                                        <input type="button" value="Go" name="go" class="button button-primary" id="CGo">
                                        <input type="hidden" value="<?php echo wp_create_nonce('customer_go'); ?>" name="customer_nonce">
                                    </td>
                                </tr>
                            </table>
                            <?php
                            $customers = generate_customers_list();

                            if($customers) {
                                $c_query = str_replace('*', 'id', $email['query']);

                                $results = $wpdb->get_results($c_query);

                                echo '<select id="customers_listing" multiple name="customers_listing[]">';

                                foreach ($customers as $customer) :
                                    if ($results) {
                                        foreach ($results as $r) {
                                            $selected = ($customer->id == $r->id) ? selected($customer->id, $r->id, false) : '';
                                            if ($selected !== '') break;
                                        }
                                    }

                                    echo '<option value="' . $customer->id . '"' . $selected . '>' . ucwords($customer->f) . ' ' . ucwords($customer->l) . '</option>';
                                endforeach;
