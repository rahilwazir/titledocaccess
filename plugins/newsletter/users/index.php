...
if ($controls->is_action('resend_welcome')) {
    ...
    $opts['confirmed_message'] = "[customer_info customer_id='" . $user->email . "']" . $opts['confirmed_message'];
...
if ($controls->is_action('remove')) {
    ...
    $wpdb->query($wpdb->prepare("delete from " . $wpdb->prefix . "customers where sid=%d", $controls->button_data));
...
<td>
    ...
    <?php $controls->button_confirm('resend', 'Send message', 'Proceed?', $s->id); ?>
